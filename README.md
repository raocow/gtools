...
| `git merge-prs` | Merge every mergeable open PR, rebasing branches that need it, skipping anything that needs a human, and listing the skips. **Safe by default: prints a plan and touches nothing until `--yes`.** Never bypasses branch protection. Processes PRs in an order that minimizes conflicts — the least-entangled PRs (fewest other open PRs touching the same files) go first — computed once from each PR's changed files, no LLM involved. For each PR: clean → merge directly; needs updating → rebase onto its base in a scratch worktree (never touches your current branch) and push, or skip if the rebase itself conflicts; blocked by required checks/reviews → always skipped. `--mine` restricts to PRs you authored; `--method merge\|squash\|rebase` overrides the merge method (default: the repo's own default). Branches aren't auto-deleted — run `git sweep` after. |
